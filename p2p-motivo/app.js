(function () {
  // ===================== Scenarios (mock "plantilla" selector, cobrador side) =====================
  const scenarios = {
    arriendo: {
      label: 'Arriendo', icon: '🏠', requesterName: 'Laura Gómez', avatar: 'LG',
      amount: 1200000, motivo: 'Arriendo apartamento — julio 2026'
    },
    servicios: {
      label: 'Servicios', icon: '💡', requesterName: 'Andrés Ruiz', avatar: 'AR',
      amount: 85000, motivo: 'Cuenta de servicios compartida (luz y agua)'
    },
    prestamo: {
      label: 'Préstamo', icon: '🤝', requesterName: 'Camila Torres', avatar: 'CT',
      amount: 150000, motivo: 'Préstamo entre amigos — viaje de fin de semana'
    },
    otro: {
      label: 'Otro', icon: '✏️', requesterName: 'Juan Pérez', avatar: 'JP',
      amount: 60000, motivo: 'Otro: regalo de cumpleaños'
    }
  };

  // ===================== Shared state (cobro P2P con motivo) =====================
  const charge = {
    amount: 0,
    template: 'arriendo',
    motivo: '',
  ...
  const chargeBubble = document.getElementById('chargeBubble');
  const bubbleRequesterName = document.getElementById('bubbleRequesterName');
  const bubbleAmount = document.getElementById('bubbleAmount');
  const bubbleMotivo = document.getElementById('bubbleMotivo');
  const chargeBubbleTime = document.getElementById('chargeBubbleTime');
  ...
  const detailMotivo = document.getElementById('detailMotivo');
  ...
  const summaryRequesterName = document.getElementById('summaryRequesterName');
  ...
  const receiptRequesterName = document.getElementById('receiptRequesterName');
  const receiptMotivo = document.getElementById('receiptMotivo');
  ...
  // ===================== Scenario ("plantilla") switcher =====================
  function loadScenario(key) {
    const s = scenarios[key];
    if (!s) return;

    charge.template = key;
    charge.amount = s.amount;
    charge.motivo = s.motivo;
    charge.requesterName = s.requesterName;
    charge.paid = false;

    scenarioList.querySelectorAll('.scenario-card').forEach(function (el) {
      el.classList.toggle('selected', el.getAttribute('data-scenario') === key);
    });

    chatAvatar.textContent = s.avatar;
    chatName.textContent = s.requesterName;

    bubbleRequesterName.textContent = s.requesterName;
    bubbleAmount.textContent = formatCOP(s.amount);
    bubbleMotivo.textContent = s.motivo;
    chargeBubbleTime.textContent = currentTime();

    detailAvatar.textContent = s.avatar;
    detailRequesterName.textContent = s.requesterName;
    detailMotivo.textContent = s.motivo;
    detailAmount.textContent = formatCOP(s.amount);

    alreadyPaidAmount.textContent = formatCOP(s.amount);

    hideAllSheets();
    hideOverlay();
    chargeBubble.hidden = false;
  ...
  scenarioList.addEventListener('click', function (e) {
    ...
    if (!card) return;
    loadScenario(card.getAttribute('data-scenario'));
  });

  loadScenario('arriendo');

  // Step 1 -> 2: open charge detail sheet
  ...
  btnAuthorizeBankPayment.addEventListener('click', function () {
    ...
    receiptRequesterName.textContent = charge.requesterName;
    receiptMotivo.textContent = charge.motivo;
